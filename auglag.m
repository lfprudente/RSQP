function [x,lambda,f,outiter,csupn,snorm,nlmannorm,time,evalfun,evalgrad,evalhess,alinfo] = auglag(n,m,x,equatn,lambda,scaling,problem,epsopt,epsfeas,epscompl,InSolver)

global evalfun evalgrad evalhess nevalal nevalnal nevalhal
...
if InSolver == 1
    fprintf('Inner solver                       : TR\n\n', epscompl)
elseif InSolver == 2
    fprintf('Inner solver                       : BFGS\n\n', epscompl)
end
if ( scaling == true )
...
while (1)
    ...
    if (csupn <= epsfeas) && (snorm <= epscompl) && (nlmannorm <= epsopt)
        ...
        fprintf('Number of AL Hessian evaluations : %i\n',nevalhal)
...
%     end
    ...
    if InSolver == 1
        problem.ehess = @(x,u,store) evalhal(x,u,store,n,m,equatn,rho,lambar,sc,scaling);
    end
...
function [rhoini] = comprhoiniAL(c,f,E,I)
